...
def my_apply_blur(image, blur_amount):
    blur_amount *= 2
...
import cv2
import numpy as np
from utils import save_visualized_images, print_tensor_info, transform

# image, label = dataset[0]
# print(f"image shape: {image.shape}")
# image1 = image[0]
# image2 = image[1]
# image = torch.cat([image1, image2], dim=0).unsqueeze(0).to(device)
# # image = image.to(device).view(1, -1, image.size(2), image.size(3))
# print(f"image shape: {image.shape}")
# save_visualized_images(image, save_dir="visualize/blur_pretrain")
# print_tensor_info(image, "Input image")
# pred_blur = model(image, mode="blur").squeeze(0)
# print(f"GT blur: {label}")
# print(f"Pred blur: {pred_blur}")
# assert 0, "Stop here"
model.train()
model.freeze_shared_layers()
dataset = BlurDataset(dataset_dir, "Q")
image, org_image = dataset[0]

gt_focal_length = 10
focal_length = 10
next_focal_length = 9
blur1 = abs(focal_length - gt_focal_length)
blur2 = abs(next_focal_length - gt_focal_length)
blurred1 = my_apply_blur(image, blur1)
blurred2 = my_apply_blur(image, blur2)

blurred1 = (blurred1.numpy() * 255).astype(np.uint8).transpose(1, 2, 0)
blurred1 = Image.fromarray(blurred1)
blurred2 = (blurred2.numpy() * 255).astype(np.uint8).transpose(1, 2, 0)
blurred2 = Image.fromarray(blurred2)

blurred1 = transform(blurred1)
blurred2 = transform(blurred2)

image = torch.cat([blurred1, blurred2], dim=0)
image = image.unsqueeze(0)
image = image.to(device)

save_visualized_images(image, save_dir="visualize/Q")
print_tensor_info(image, "Input image")
pred_blur = model(image, mode="blur").squeeze(0)  # 예측 결과
print(f"GT blur: {blur1}, {blur2}")
print(f"Pred blur: {pred_blur[0]:.4f}, {pred_blur[1]:.4f}")
assert 0, "Stop here"
