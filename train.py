...
import random
from utils import load_images, apply_blur, calculate_reward, ReplayBuffer, save_checkpoint, compute_loss
...
def train_dqn(num_episodes=100000, image_batch_size=1, replay_batch_size=256, gamma=0.99, epsilon_start=0.7, 
    ...
    for episode in range(num_episodes):
        ...
        states = [torch.cat([image_0s[i], image_1s[i]], dim=0) for i in range(image_batch_size)]
        ...
        for step in range(10):
            ...
            next_states = [torch.cat([image_1s[i], new_images[i]], dim=0) for i in range(image_batch_size)]
